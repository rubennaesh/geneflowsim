...
st.set_page_config(page_title="GeneFlowSim", layout="wide")
...
def simulate_and_plot(p, q, gens):
    # Generation 0 calculation
    p2 = p ** 2
    two_pq = 2 * p * q
    q2 = q ** 2
...
if st.session_state.tries > 0:
    ...
    if st.session_state.tries == 0:
        ...
        st.image("https://via.placeholder.com/200x200.png?text=Scan+to+Pay", width=200)
else:
    st.success("Unlimited trials unlocked.")
    st.markdown("_You have unlimited access._")
    # Repeat simulation UI for unlimited access
    simulate_and_plot(p, q, gens)
